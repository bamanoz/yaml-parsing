...
Minimal files needed in a repository:
...
- `docker-compose.yml` builds the Tabula image and mounts the repo at
  `/workspace`.
- `tabula.app.toml` declares the app id, distro source, workspace binding, and
  runtime topology.
- `.tabula/config/global.toml` selects the driver provider and provider config.
- `.tabula/secrets.json` stores provider API keys referenced by
  `global.toml`.

The rest of `.tabula/` is generated by `tabula-install` inside the container:
installed distros, bundles, tenant config, run files, logs, sessions, and cache.
Do not commit `.tabula/`.
